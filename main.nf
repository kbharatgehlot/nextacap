...
process ImageWithWSClean { 
    ...
    """
    wsclean -interval 0 120 -data-column ${datacolumn} -gridder wgridder -wgridder-accuracy 1e-5 -reorder -make-psf -scale ${scale} -size ${size} ${size} -weight ${weight} -minuv-l ${minuv_lambda} -maxuv-l ${maxuv_lambda} -pol ${polarisation} -name ${name} -j ${threads} ${mses}  > ${params.logs_dir}/wsclean.log 2>&1
