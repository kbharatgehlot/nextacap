...
parser.add_argument('-f','--clipvalue',help='clipvalue, default=70',dest="clipvalue", type=float,default=70)
parser.add_argument('-c','--column', help='column default=DATA',dest='column',default='DATA')
parser.add_argument('-o','--outputcolumn', help='out put column default=MODEL_DATA',dest='outputcolumn',default='MODEL_DATA')
parser.add_argument('-r','--reset_flags', help='set all flags to false',action='store_true')
parser.add_argument('-t','--flag_intrastations', help='flag all intrastations',action='store_true')
parser.add_argument('-l','--flag_longbaselines', help='flag stations outside 10km of core',action='store_true')
parser.add_argument('-b','--flag_badbaselines', help='flag bad CS CS stations',action='store_true')
parser.add_argument('-m','--flag_remote', help='flag all remote stations',action='store_true')
parser.add_argument('-n','--do_not_clip', help='do not clip nor create data columns',action='store_true')
    ...
    for  MSlist in MSlists:
        for i in MSlist:
            ...
            if args.flag_intrastations:
                ...
                tab.taql(r'UPDATE $i SET FLAG=True WHERE mscal.baseline("/(.*)HBA0&\1HBA1/")')#mind the r
            if args.flag_longbaselines:
                ...
                if freq<134e6:
                    tab.taql(r'UPDATE $i SET FLAG=True WHERE mscal.baseline("RS406HBA")')#mind the r
                elif freq<146e6:
                    tab.taql(r'UPDATE $i SET FLAG=True WHERE mscal.baseline("RS306HBA,RS406HBA")')#mind the r
                else:
                    tab.taql(r'UPDATE $i SET FLAG=True WHERE mscal.baseline("RS406HBA,RS306HBA,RS106HBA")')#mind the r

                #tab.taql(r'UPDATE $i SET FLAG=True WHERE mscal.baseline(">10000")')#mind the r
            if args.flag_badbaselines:
                print("flagging bad CS-CS baselines, only flagging CS013 when needed now")
                obsid = int(i.split("/")[-1].split("_")[0][1:])
                if obsid<100000:
                    print("obsid %d:  flagging bad CS013"%obsid)
                    tab.taql(r'UPDATE $i SET FLAG=True WHERE mscal.baseline("CS013HBA0,CS013HBA1")')
            ...
            if args.flag_remote:
                ...
                tab.taql(r'UPDATE $i SET FLAG=True WHERE mscal.baseline("R*&&*")')#mind the r
