...
data = rev.get_data(args.obsid)  # "L254871_flagged"
data.do_flag()


ps_gen = data.get_ps_gen(
    rmean_freqs=True, umax=250, umin=50, du=8, window_fct="hann", eor_bin_name="3"
)
...
ps_gen.get_ps2d(data.i).plot(
    ax=ax1, title="PS2D Stokes I", vmin=2e1, vmax=1e4, wedge_lines=[90], z=ps_gen.eor.z
)
ps_gen.get_ps2d(data.v).plot(
    ax=ax2, title="PS2D Stokes V", vmin=2e1, vmax=1e4, wedge_lines=[90], z=ps_gen.eor.z
)
fig.tight_layout()
plt.savefig(f"{args.obsid}_IV_power_spectrum.png", bbox_inches="tight")
plt.close()
